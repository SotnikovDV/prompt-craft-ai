...
<body class="font-sans antialiased">
    <div class="flex h-screen relative">
        ...
        <div id="sidebar" class="relative bg-gray-50/90 backdrop-blur-sm border-r border-gray-200 transition-all duration-300 ease-in-out hover:w-80 w-16 group flex flex-col h-full md:relative md:z-auto z-50 md:shadow-none shadow-lg">
            <!-- Логотип -->
            <div class="p-4 border-b border-gray-200 flex-shrink-0">
                <div class="w-full flex items-center justify-center group-hover:justify-start">
                    ...
                    <span class="ml-3 text-lg font-bold text-gray-900 hidden group-hover:block">Толкователь ИИ</span>
                </div>
            </div>
            ...
            <div class="p-2 space-y-1 flex-shrink-0">
                ...
                <button id="new-chat-btn" onclick="startNewChat()"
                    class="w-full flex items-center justify-center p-3 hover:bg-gray-100 rounded-lg transition-colors group-hover:justify-start"
                    title="Новый запрос">
                    <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                    </svg>
                    <span class="ml-3 text-sm font-medium text-gray-700 hidden group-hover:block">Новый запрос</span>
                </button>

                <!-- Поиск -->
                <button class="w-full flex items-center justify-center p-3 hover:bg-gray-100 rounded-lg transition-colors group-hover:justify-start"
                    title="Поиск в чатах">
                    <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                    </svg>
                    <span class="ml-3 text-sm font-medium text-gray-700 hidden group-hover:block">Поиск в чатах</span>
                </button>
                ...
                <button class="w-full flex items-center justify-center p-3 hover:bg-gray-100 rounded-lg transition-colors group-hover:justify-start"
                    ...
                    <span class="ml-3 text-sm font-medium text-gray-700 hidden group-hover:block">Библиотека</span>
                </button>

                <!-- История чатов (только иконка в свернутом виде) -->
                <div class="w-full flex items-center justify-center p-3 hover:bg-gray-100 rounded-lg transition-colors group-hover:justify-start"
                    title="История чатов">
                    <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                    <span class="ml-3 text-sm font-medium text-gray-700 hidden group-hover:block">История чатов</span>
                </div>
            </div>

            <!-- Список истории запросов (только в развернутом виде) -->
            <div id="chat-history-section" class="flex-1 overflow-y-auto min-h-0 group-hover:block hidden sidebar-open:block">
                <div class="px-2 py-1">
                    <h3 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Чаты</h3>
                </div>
                <div id="chat-history" class="p-2 space-y-1">
                    <!-- Загрузка истории -->
                    <div id="history-loading" class="p-4 text-center">
                        <svg class="w-5 h-5 animate-spin text-gray-400 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-4 border-t border-gray-200 flex-shrink-0 relative" style="margin-top: auto;">
                <button id="user-profile-btn" class="w-full flex items-center justify-center group-hover:justify-start hover:bg-gray-100 rounded-lg p-2 transition-colors"
                    ...
                    <div class="ml-3 hidden group-hover:block flex-1 min-w-0">
                ...
                <div id="user-menu" class="absolute bottom-full left-4 right-4 mb-2 bg-white border border-gray-200 rounded-lg shadow-lg hidden z-50">
                    <div class="p-2">
                        <!-- Профиль -->
                        <a href="{{ route('profile.edit') }}" class="flex items-center px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">
                        ...
                        <!-- Обновить план -->
                        <button class="w-full flex items-center px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">
                            <svg class="w-4 h-4 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <form method="POST" action="{{ route('logout') }}" class="w-full">
                            @csrf
                            <button type="submit" class="w-full flex items-center px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">
                                <svg class="w-4 h-4 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    function shareChat() {
        if (navigator.share) {
            navigator.share({
                title: 'Толкователь ИИ',
                text: 'Попробуйте Толкователь ИИ для создания профессиональных промптов',
            ...
            navigator.clipboard.writeText(window.location.href).then(() => {
                if (window.infoModalManager) {
                    window.infoModalManager.showSuccess('Ссылка скопирована!', 'Ссылка на чат скопирована в буфер обмена');
                }
            });
        }
    }
                ...
                document.addEventListener('click', function(event) {
                    ...
                    if (userMenu && userProfileBtn && !userMenu.contains(event.target) && !userProfileBtn.contains(event.target)) {
